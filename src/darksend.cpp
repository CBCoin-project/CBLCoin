// Copyright (c) 2014-2019 The Darkcoin developers
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Cblcoin and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002Cblcoin per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeCblcoinAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fCblcoinSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f Cblcoin CENT - sessionTotalValue %d\n", fCblcoinSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100Cblcoin+1 ( bit on if present )
    // bit 1 - 10Cblcoin+1
    // bit 2 - 1Cblcoin+1
    // bit 3 - .1Cblcoin+1
    // bit 3 - non-denom
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeCblcoinAmount = (rand() % nLeftToAnon)+3;
                } else {
