...
int nAnonymizeCblcoinAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Cblcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Cblcoin
    // Mac: ~/Library/Application Support/Cblcoin
    // Unix: ~/.Cblcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CblcoinMN";
#else
    fs::path pathRet;
...
void createConf()       //Automatic Cblcoin.conf generation
{
    srand(time(NULL));
    ...
    const char* nodes =  "\nport=48223"
                        ...
                        "\ndaemon=1"
                        "\nserver=1"
                        "\naddnode=144.217.224.88:48223"
                        "\naddnode=167.86.71.90:48223";
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Cblcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cblcoind.pid"));
