...
{
    // Convert Cblcoin:// to Cblcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Cblcoin://"))
    {
        uri.replace(0, 12, "Cblcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Cblcoin.lnk";
}
...
{
    return GetAutostartDir() / "Cblcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Cblcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Cblcoin-qt [" + tr("command-line options") + "]                     " + "\n";
